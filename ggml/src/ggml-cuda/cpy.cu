...
// Non-contiguous same-type copy for quantized tensors. Matches the CPU
// ggml_compute_forward_dup_bytes behavior: walk src and dst blocks in
// parallel by the linear block index N, using each tensor's own dim-0
// block count. Handles reshape copies where ne00 != ne10.
//
// Fixes upstream bug where iq4_nl->iq4_nl strided copies (scheduled by
// MLA KV views on Mistral Large 3 / Kimi K2.5) fell through to GGML_ABORT.
static __global__ void cpy_blck_quant_same(
        const char * __restrict__ cx, char * __restrict__ cdst,
        const int64_t num_blocks,
        const int64_t nk00, const int64_t ne01, const int64_t ne02, const int64_t ne03,
        const int64_t nb00, const int64_t nb01, const int64_t nb02, const int64_t nb03,
        const int64_t nk10, const int64_t ne11, const int64_t ne12, const int64_t ne13,
        const int64_t nb10, const int64_t nb11, const int64_t nb12, const int64_t nb13,
        const int type_size) {
    const int64_t N = (int64_t)blockDim.x*blockIdx.x + threadIdx.x;
    if (N >= num_blocks) return;

    // src block coords derived from N in src's block layout
    const int64_t k00 = N % nk00;
    const int64_t i01 = (N / nk00) % ne01;
    const int64_t i02 = (N / (nk00 * ne01)) % ne02;
    const int64_t i03 = (N / (nk00 * ne01 * ne02)) % ne03;
    const int64_t src_offset = k00*nb00 + i01*nb01 + i02*nb02 + i03*nb03;

    // dst block coords from the same N in dst's (potentially different) block layout
    const int64_t k10 = N % nk10;
    const int64_t i11 = (N / nk10) % ne11;
    const int64_t i12 = (N / (nk10 * ne11)) % ne12;
    const int64_t i13 = (N / (nk10 * ne11 * ne12)) % ne13;
    const int64_t dst_offset = k10*nb10 + i11*nb11 + i12*nb12 + i13*nb13;
...
static void ggml_cpy_quant_same_cuda(
    ...
    const int64_t nk00 = ne00 / qk;
    const int64_t nk10 = ne10 / qk;
    const int64_t num_blocks = nk00 * ne01 * ne02 * ne03;
    GGML_ASSERT(num_blocks == nk10 * ne11 * ne12 * ne13);
    const int threads = 256;
    const int64_t grid = (num_blocks + threads - 1) / threads;
    GGML_ASSERT(grid < UINT_MAX);
    cpy_blck_quant_same<<<grid, threads, 0, stream>>>(
            cx, cdst, num_blocks,
            nk00, ne01, ne02, ne03, nb00, nb01, nb02, nb03,
            nk10, ne11, ne12, ne13, nb10, nb11, nb12, nb13,
            type_size);
}
